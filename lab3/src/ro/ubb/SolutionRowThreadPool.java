...
public class SolutionRowThreadPool {
    ...
    public static void main(String[] args) {
        ...
        float start, end;
        try (PrintWriter output = new PrintWriter(new BufferedWriter(new FileWriter("testResultRowExecutorService.txt", true)))) {

            for (int i = 0; i < tasks.size(); i++) {
                start = System.nanoTime() / 1000000;
                performTest(tasks.get(i));
                end = System.nanoTime() / 1000000;
                String resultString = "End work multiplication row with thread pool: " + (end - start) / 1000 + " seconds, " + tasks.get(i).getThreadCount() + " threads/tasks and matrix of size " + tasks.get(i).getRowSize() + "x" + tasks.get(i).getColumnSize();
    ...
    private static void performTest(TaskSimple taskSimple) {
        ...
        for (int i = 0; i < taskSimple.getThreadCount() && i < numberOfTotalPositions; i++) {
            ...
            pool.execute(new Thread(new MultiplicationRows(a, b, result, si, sj, val)));
