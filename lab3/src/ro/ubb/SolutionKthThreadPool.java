...
public class SolutionKthThreadPool {
    public static void main(String[] args) {
        ...
        float start, end;
        try (PrintWriter output = new PrintWriter(new BufferedWriter(new FileWriter("testResultEveryKthExecutorService.txt", true)))) {

            for (int i = 0; i < tasks.size(); i++) {
                start = System.nanoTime() / 1000000;
                performTest(tasks.get(i));
                end = System.nanoTime() / 1000000;
                String resultString = "End work multiplication every kth element with thread pool: " + (end - start) / 1000 + " seconds, " + tasks.get(i).getThreadCount() + " threads/tasks and matrix of size " + tasks.get(i).getRowSize() + "x" + tasks.get(i).getColumnSize();
    ...
    private static void performTest(TaskSimple taskSimple) {
        ...
        for (int i = 0; i < taskSimple.getThreadCount() && si < taskSimple.getRowSize(); i++) {
            pool.execute(new Thread(new MultiplicationByKth(a, b, result, si, sj, taskSimple.getThreadCount())));
